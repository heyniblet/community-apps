"""
Applet: SkordleSlowPitch
Summary: Displays SP Games
Description: The app gets slow pitch data from the Skordle website to display on the Tidbyt. A user can select the class of game and manually select which game to display.
...
CLASSES = {"6A": 225, "5A": 226, "4A": 227, "3A": 228, "2A": 229, "A": 230, "Other": 231}
        ...
        return render.Root(
            child = render.Box(
                child = render.WrappedText(
                    content = "No Events for {} Slow Pitch".format(sportClass),
                    width = 60,
                    linespacing = 1,
                    font = "CG-pixel-3x5-mono",
                ),
...
def get_data(classID):
    web = http.get("https://skordle.com/scores/?sportid=5&classid={}&clubid=1".format(classID), ttl_seconds = 60)
