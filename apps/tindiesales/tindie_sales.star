...
def main(config):
    ...
    error = None
    if config.str("username", DEFAULT_USERNAME) == DEFAULT_USERNAME or config.str("api_key", DEFAULT_KEY) == DEFAULT_KEY:
        ...
        for _ in range(20):
            ...
            if query.status_code == 200:
                ...
                if meta.get("next") and meta["next"].startswith("/api/v1/order/"):
