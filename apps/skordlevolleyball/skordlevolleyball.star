"""
Applet: SkordleVolleyball
Summary: Displays Skorld VB Games
Description: This app gets Volleyball data from the Skordle website to dplay on the Tidbyt. A user can select the class of game and manually select which game to display.
...
CLASSES = {"6A": 429, "5A": 430, "4A": 431, "3A": 432, "Other": 433}
        ...
        return render.Root(
            child = render.Box(
                child = render.WrappedText(
                    content = "No Events for {} Volleyball".format(sportClass),
                    width = 60,
                    linespacing = 1,
                    font = "CG-pixel-3x5-mono",
                ),
...
def get_data(classID):
    web = http.get("https://skordle.com/scores/?sportid=7&classid={}&clubid=1".format(classID), ttl_seconds = 60)
