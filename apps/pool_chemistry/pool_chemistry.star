...
def fetch(ha_url, token, entity_id):
    ...
    if r.status_code != 200:
        return -1.0
    body = r.body()
    if len(body) > MAX_RESPONSE_BYTES or not body.startswith("{") or not body.endswith("}"):
        return -1.0
...
def ph_color(v):
    return color_for(v, 7.2, 7.3, 7.5, 7.6)

def fc_color(v):
    return color_for(v, 1.0, 1.5, 5.0, 5.0)

def ta_color(v):
    return color_for(v, 80, 90, 110, 120)

def cya_color(v):
    return color_for(v, 30, 40, 70, 80)

def ca_color(v):
    return color_for(v, 150, 175, 275, 300)
...
def main(config):
    ha_url = home_assistant_url(config.get("ha_url", ""))
            ...
            child = render.Box(
                ...
                child = render.Column(
                    ...
                    children = [
                        ...
                        render.Box(height = 3),
                        render.Text("Set HTTPS URL+Token", font = "tom-thumb", color = DIM),
                    ],
                ),
            ),
        )

    pool_id = pool_slug(config.get("pool_id", "my_pool"))
    if not pool_id:
        return render.Root(child = render.WrappedText("Check the pool entity slug", align = "center"))
    ...
    return schema.Schema(
        ...
        fields = [
            schema.Text(
                ...
                desc = "Your public HTTPS HA base URL, e.g. https://ha.example.com",
                icon = "house",
            ),
            schema.Text(
                id = "ha_token",
