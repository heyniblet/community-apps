"""
Applet: Skordle BB Fem
Summary: Displays Female BB games
Description: This app gets female basketball data from the Skordle website to display on the Tidbyt. A user can select the class of game and manually select which game to display.
        ...
        return render.Root(
            child = render.Box(
                child = render.WrappedText(
                    content = "No Events for Women's Basketball",
                    width = 60,
                    linespacing = 1,
                    font = "CG-pixel-3x5-mono",
                ),
...
def get_data():
    web = http.get("https://skordle.com/scores/?sportid=31&clubid=1", ttl_seconds = 60)
...
def get_schema():
    game_options = [schema.Option(display = "{}".format(game), value = "{}".format(game)) for game in range(1, 51)]
