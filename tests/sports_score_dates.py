"""Run: python3 tests/sports_score_dates.py /path/to/niblet (no network)."""
...
def event(identity, state, team="TEST"):
    return {"id": identity, "status": {"type": {"state": state}}, "competitions": [
        {"competitors": [{"team": {"abbreviation": team}}, {"team": {"abbreviation": "OTHER"}}]}
    ]}


with tempfile.TemporaryDirectory() as temporary:
    for league in ("mlb", "nba", "wnba"):
...
''' + tail
        ...
        for day, instant in (
            ...
            (date(2026, 10, 31), "2026-10-31T04:30:00Z"),
        ):
            fixtures = {"limit=100": [event("all-a", "pre"), event("all-b", "pre", "ELSE")]}
...
def main(config):
    ...
    for team, expected in [
        ...
        actual = [score["id"] for score in get_scores(now, team)]
        if actual != expected:
