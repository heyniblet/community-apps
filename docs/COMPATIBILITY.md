...
The original audit snapshot pinned upstream Pixlet v0.53.1. The current CI setup pins Niblet CLI v0.54.12 for sports apps using `frame_keys`, which upstream Pixlet cannot evaluate. Those apps therefore have an additional runtime compatibility limitation. CI does not run a complete Niblet/Tronbyt matrix. Focused audit probes loaded seven selected apps' schemas from both snapshots on upstream Pixlet v0.53.1 and v0.54.0. Network-denied probes reproduced the Roomba and Fitbit issues above. These checks do not certify the whole collection.
