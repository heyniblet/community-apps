...
When importing, retain upstream human authorship and source provenance. Credit Edwin for integration and downstream modifications, not for original upstream contributions. Preserve original notices, and identify any adapted patch and source commit in the contribution record. New commit messages follow repository rules on attribution trailers.
...
The workflow compares against the merge base of the push's old and new heads. Rewritten history made that merge base the September 3 shared ancestor. However, both the merge-base diff and a direct old/new tree diff select the same 744 directories in this case, because the maintenance update added app README and source notices throughout that set. A comparison-base fix alone would not resolve this failure.
...
2. Make push comparisons use the before/after trees and keep merge-base comparison for pull requests. Cover rewritten-history behavior in the existing target-selection tests.
